...
describe("POST /api/agent-runs", () => {
  ...
  it("propagates a real Node HTTP client abort to the executor and stops controlled work", async () => {
    ...
    try {
      ...
      await withTimeout(
        new Promise<void>((resolve) => {
          const checkForTerminalTelemetry = () => {
            if (
              logs.some(
                (record) =>
                  record.attributes["agent.run.id"] === "ar_http_abort" &&
                  record.eventName === "agent.run.cancelled",
              )
            ) {
              resolve();
              return;
            ...
            setTimeout(checkForTerminalTelemetry);
          };

          checkForTerminalTelemetry();
        }),
        "Timed out waiting for terminal cancellation telemetry",
      );
      ...
      ).toEqual(["agent.run.accepted", "agent.run.cancellation_requested", "agent.run.cancelled"]);
    } finally {
      await server.close();
    }
  });
